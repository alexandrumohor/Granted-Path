...
interface DashboardData {
  ...
  activeCourses: { title: string; slug: string; progress: number; nextModule: string; nextLesson: string; estimatedMinutes: number }[];
  goals: { id: string; title: string; targetDate: string | null; progress: number }[];
  weekStats: { minutes: number; xp: number; streak: number; lessonsCompleted: number };
}
...
export default function DashboardPage() {
  ...
  const otherCourses = data?.activeCourses.slice(1) ?? [];
  const weekStats = data?.weekStats ?? { minutes: 0, xp: 0, streak: 0, lessonsCompleted: 0 };
  ...
  return (
    <div className="p-4 lg:p-8 max-w-[1200px] mx-auto">
      ...
      {activeCourse ? (
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-6">
            <div className="flex-1">
              ...
              <p className="mt-1 text-[14px] text-muted-foreground">
                {activeCourse.nextModule} → <span className="text-foreground font-medium">{activeCourse.nextLesson}</span>
              </p>
              <div className="mt-4 flex items-center gap-4">
                ...
                {activeCourse.estimatedMinutes > 0 && (
                  <div className="flex items-center gap-1.5 text-[13px] text-muted-foreground">
                    <Clock className="h-3.5 w-3.5" />
                    <span>{activeCourse.estimatedMinutes} min</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="rounded-xl border border-dashed border-border bg-card/50 p-8 mb-6 text-center">
          <BookOpen className="mx-auto h-10 w-10 text-muted-foreground/30 mb-3" />
          <p className="text-[16px] font-medium">{t("noCourses")}</p>
          <p className="text-[14px] text-muted-foreground mt-1">{t("noCoursesDesc")}</p>
          <Link href="/learn">
                  ...
                  <Link key={i} href={`/learn/${c.slug}`}>
                    <div className="rounded-xl border border-border bg-card p-4 hover:border-primary/30 transition-all duration-150 hover:translate-y-[-1px] flex items-center gap-4" style={{boxShadow:"var(--shadow-card)"}}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[12px] text-muted-foreground mt-0.5">{c.nextLesson}</p>
                      </div>
                      <div className="w-28 shrink-0 hidden sm:block">
                        <div className="flex items-center justify-between text-[12px] mb-1">
                          <span className="text-muted-foreground">{c.progress}%</span>
                          {c.estimatedMinutes > 0 && (
                            <span className="text-muted-foreground flex items-center gap-1"><Clock className="h-3 w-3"/>{c.estimatedMinutes} min</span>
                          )}
                        </div>
            ...
            <div className="rounded-xl border border-border bg-card p-5" style={{boxShadow:"var(--shadow-card)"}}>
              <div className="grid grid-cols-4 gap-4">
                <div className="text-center">
                  ...
                  <p className="text-[20px] font-bold">{weekStats.minutes}<span className="text-[13px] font-normal text-muted-foreground">m</span></p>
                  <p className="text-[11px] text-muted-foreground mt-0.5">{tc("minutes")}</p>
                </div>
                <div className="text-center">
                  <div className="flex items-center justify-center gap-1.5 mb-1">
                ...
                <div className="text-center">
                  ...
                  <p className="text-[20px] font-bold">{weekStats.lessonsCompleted}</p>
                  <p className="text-[11px] text-muted-foreground mt-0.5">{tc("lessons")}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/goals" className="text-[13px] text-primary hover:underline">{tc("viewAll")}</Link>
            </div>
            {goals.length > 0 ? (
              <div className="space-y-3">
                {goals.map(g => (
                  <div key={g.id} className="rounded-xl border border-border bg-card p-4" style={{boxShadow:"var(--shadow-card)"}}>
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-start gap-2.5">
                        <Target className="h-4 w-4 text-primary mt-0.5 shrink-0" />
                        <div>
                          <p className="text-[13px] font-medium">{g.title}</p>
                          {g.targetDate && <p className="text-[11px] text-muted-foreground mt-0.5">{g.targetDate}</p>}
                        </div>
                      </div>
              ...
              <div className="rounded-xl border border-dashed border-border bg-card/50 p-6 text-center">
                ...
                <Link href="/goals">
                  <Button variant="outline" size="sm" className="mt-3">{t("addGoal")}</Button>
                </Link>
              </div>
            )}
          </div>
